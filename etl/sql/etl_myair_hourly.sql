-- =========================================================
-- etl_myair_hourly.sql
--
-- Materialized hourly aggregation of myair_5min.
--
-- One row per device, firmware and fixed hourly bucket.
-- Hourly tables are built from five-minute tables rather
-- than directly from tidy observations.
--
-- Temporal weighting:
--   each available five-minute interval contributes one
--   equally weighted value to the hourly mean, regardless
--   of how many raw readings were recorded in that interval.
--
...
--
-- records_n:
--   total tidy records represented by the hourly row
--
-- five_min_n:
--   total five-minute rows represented, with a maximum
--   expected value of 12 for a complete hour.
--
-- Only bucket_hour is stored as temporal information.
-- Date and hour can be derived when needed.
-- =========================================================
...
CREATE TABLE IF NOT EXISTS myair_hourly (
  ...
  INDEX idx_myair_hourly_user_bucket (
    userId,
    bucket_hour
  ),

  INDEX idx_myair_hourly_bucket (
    bucket_hour
  )
...
BEGIN
  ...
  -- Full rebuild of the materialized hourly table
  DROP TABLE IF EXISTS myair_hourly;

  CREATE TABLE myair_hourly (
